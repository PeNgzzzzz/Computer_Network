...
def install_udp_middlebox_flow( source_dpid
    ...
    # Get the topology network
    topology_graph = get_networkx_topology_graph()
    
    # Get paths from source to middlebox and from middlebox to destination
    path_to_middlebox = get_shortest_path_between(topology_graph, source_dpid, middlebox_dpid)
    path_from_middlebox = get_shortest_path_between(topology_graph, middlebox_dpid, destination_dpid)

     # Get MAC and IP addresses of source and destination
    ...
    # Install flow rules for the path from source to middlebox
    for i in range(len(path_to_middlebox)):
        src = path_to_middlebox[i]
        if i != len(path_to_middlebox) - 1: dst = path_to_middlebox[i + 1]
        # Get in_port
        in_port = get_input_port(path_to_middlebox, src)
        # Build a flow rule for inputs to match
        match = build_match_for(source_mac, destination_mac, source_ip, destination_ip, source_port, destination_port, in_port)
        # Build a flowmod to output
        if i != len(path_to_middlebox) - 1: flowmod = build_openflow_flowmod(match, get_ports_connecting(src, dst)[0])
        else: flowmod = build_openflow_flowmod(match, get_host_port(src))  # The last switch in the path
        # Install the flowmod
        install_flowmod_on_switch_with_dpid(flowmod, src) 

    # Install flow rules for the path from middlebox to destination
    for i in range(len(path_from_middlebox) - 1):
        src = path_from_middlebox[i]
        if i != len(path_from_middlebox) - 1: dst = path_from_middlebox[i + 1]
        # Get in_port
        in_port = get_input_port(path_from_middlebox, src)
        # Build a flow rule for inputs to match
        match = build_match_for(source_mac, destination_mac, source_ip, destination_ip, source_port, destination_port, in_port)
        # Build a flowmod to output
        if i != len(path_from_middlebox) - 1: flowmod = build_openflow_flowmod(match, get_ports_connecting(src, dst)[0])
        else: flowmod = build_openflow_flowmod(match, get_host_port(src))  # The last switch in the path
        # Install the flowmod
        install_flowmod_on_switch_with_dpid(flowmod, src) 
